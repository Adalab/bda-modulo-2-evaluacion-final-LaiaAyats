...
FROM
	customer c	
-- Asegurar que se mantienen todos los clientes aunque no hayan alquilado películas. Como no hay ningún cliente sin alquiler, también se podría utilizar el INNER JOIN.  
LEFT JOIN 
...
ORDER BY
	last_name, 
	first_name; -- Plus para ordenar los actores alfabéticamente (apellido).
...
WHERE 	
	fa.actor_id IS NULL; 
			...
			SELECT 
				'Total Actors' AS tables, COUNT(*) AS count
			FROM 
				actor
			UNION ALL
			SELECT 
				'Actors in Films' AS tables, COUNT(DISTINCT actor_id) AS count
			FROM 
				film_actor;
...
-- Ejercico 25. BONUS: Encuentra todos los actores que han actuado juntos en al menos una película. La consulta debe mostrar el nombre y apellido de los actores y el número de películas en las que han actuado juntos.
